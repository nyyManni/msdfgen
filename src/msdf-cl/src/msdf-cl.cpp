...
void add_segment(segment *, segment *, segment *, int, int, vec2);
...
void calculate_pixel(struct shape *shape, vec3 *output, int x, int y, int stride,
    ...
    for (int _i = 0; _i < shape->ncontours; ++_i) {
        ...
        for (int _i = 0; _i < c->nsegments; ++_i) {
            add_segment(prev, cur, s, cur->npoints, cur->color, p);
            prev = cur;
            cur = s;
            NEXT_SEGMENT(s);
        }

        set_contour_edge(c->winding, p);
...
void add_segment(segment *prev, segment *cur, segment *next,
    ...
    segment_distance d = signed_distance(cur, npoints, point);
